...
export const OTPLoginForm: React.FC<OTPProps> = ({ onLoginSuccess }) => {
  ...
  const { mutate: sendOtpCode, isLoading: loading, hasOTP, otpId, setHasOTP } = useSendOtpCode();
  const { mutate: otpLogin, isLoading: otpLoginLoading, serverError, setServerError } =
    useOtpLogin();
  ...
  function onOtpLoginSubmission(values: OtpLoginInputType) {
    otpLogin({
      ...values,
      phone_number: number,
      otp_id: otpId,
